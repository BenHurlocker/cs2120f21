...
def reflexive := ∀ x, x ≺ x -- everything is related to itself (like equals)
def anti_reflexive := ∀ x, ¬ x ≺ x -- nothing is related to itself (like less than -- no object is less than itself)
def irreflexive := anti_reflexive r -- sometimes used
def anti_symmetric := ∀ ⦃x y⦄, x ≺ y → y ≺ x → x = y -- like less than or equal -- there is an object that equals itself
def asymmetric := ∀ ⦃x y⦄, x ≺ y → ¬ y ≺ x -- less than or greater than -- x can be greater than y but then y cannot be greater than x
...
example : reflexive r → ¬ asymmetric r := _   -- true? No, false.
